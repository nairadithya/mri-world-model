...
- **D42 — Separate assessment from forecasting and reset the validity gate
  (2026-09-14).** ASTRA/K3 showed that observed-pair response assessment
  (`x_t + x_{t+1} → RANO_{t+1}`), past-only forecasting (`H_t → future`),
  anatomy forecasting, and surprise detection are different tasks. Every new
  comparison must use an immutable row ID, explicit input cutoff, clean label
  contract, and task-specific metric. The historical `final` split is
  development evidence, not an untouched holdout; eventual overall survival
  is excluded from prospective inputs; corrected AUCs and current-schema
  caches are required before reusing old surprise or site numbers. See
  `info/p0_audit.md`, `info/lumiere_task_manifest.json`, and
  `info/eval_protocol.md`. Status: implementation complete; cache rebuild,
  independent SAILOR codebook verification, timing reconciliation, and a new
  external/hidden holdout remain blocked.

- **D43 — No broad JEPA sweep before a matched benchmark (2026-09-14).** The
  next program stage is CPU-first: finish the survival-free prospective
  baselines, then reproduce a paired-scan radiomics/measurement assessment
  baseline and test whether history/JEPA adds an incremental gain. Forecasting
  is evaluated separately against last-status, transition, growth-trend, and
  small temporal baselines. Only after those gates pass is a lesion-aware,
  externally supervised forecast worth GPU work. External cohorts are assigned
  distinct roles: UCSF for lesion/change supervision, Burdenko for longer
  trajectories, UCSD-PTGBM for treatment-effect discrimination, and SAILOR
  for developmental transfer rather than a pristine final test.

- **D44 — Freeze the refreshed P0 readout policy (2026-09-21).** The
  current-schema locked-unseen scorecard is regenerated by
  `scripts/p0_scorecard.py` from `checkpoints/p0_probe_cache.pt`, with pooled
  and patient-uniform metrics, 10,000 patient-cluster bootstrap resamples,
  paired comparisons, and a 3-seed × 3-width sensitivity grid. Development
  readouts are frozen to the class-weighted MLP, hidden width 256, seed 42,
  without feature standardization. The historical `final` slice remains
