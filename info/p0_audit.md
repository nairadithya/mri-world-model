...
The complete refreshed local classification scorecard is generated by
`scripts/p0_scorecard.py` and stored in the ignored artifact
`outputs/p0_scorecard.json`. It covers the locked 26-patient unseen cohort,
reports pooled and patient-uniform metrics, uses 10,000 patient-cluster
bootstrap resamples, and records paired comparisons plus a 3-seed × 3-width
readout sensitivity grid. The frozen development readout policy is MLP,
hidden width 256, seed 42, without feature standardization.

The primary forecast comparison is `states_forecast` versus `fused`:
macro-F1 **0.2718 vs 0.2095 pooled** and **0.2247 vs 0.1428
patient-uniform**, paired difference **+0.0623 [0.0003, 0.1273]**. This is a
development result on the locked unseen cohort, not an external headline.
